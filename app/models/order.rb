class Order < ActiveRecord::Base
	...
	class << self
		def make(sender_id, receiver, staff, pay_form_receiver, goods, location_id, location_type)
			raise(ParameterError, 'location_type') unless ['ShopAddress', 'SpecifyAddress', 'StoreAddress'].include?(location_type)
			...
			transaction do
				...
				order = create!(sender_id: sender_id, receiver: receiver, staff: staff, pay_form_receiver: pay_form_receiver)
				...
				goods.collect! do |x|
					...
					CheckLog.create!(time: Date.current, good: good, location: location, check_action: check_action, staff: staff)
					good.id
				end
